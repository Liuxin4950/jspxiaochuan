...
var refreshItem = function(){
    ...
    var target = $('.Top_iframe[data-id="' + currentId + '"]', topWindow);
...
var closeItem = function(dataId){
	...
	if($.common.isNotEmpty(dataId)){
	    ...
	    // 移除相应tab对应的内容区
	    $('.mainContent .Top_iframe[data-id="' + dataId + '"]', topWindow).remove();
	    return;
	}
	...
	if($.common.isNotEmpty(panelUrl)){
	    ...
	    $('.mainContent .Top_iframe', topWindow).each(function() {
	        if ($(this).data('id') == panelUrl) {
	            $(this).show().siblings('.Top_iframe').hide();
	            return false;
            }
        });
    }
...
function createMenuItem(dataUrl, menuName) {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="Top_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-panel="' + panelUrl + '" seamless></iframe>';
        $('.mainContent', topWindow).find('iframe.Top_iframe').hide().parents('.mainContent').append(str1);
